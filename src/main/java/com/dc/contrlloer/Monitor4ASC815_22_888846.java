...
public class Monitor4ASC815_22_888846 {
...
//				}
			...
			for (StockCode result : Results) {
				...
				if (resultList2Days != null && resultList2Days.size() > 0) {
					HistorytradeInfo historytradeInfo = resultList2Days.get(0);
					Scpool scpool = new Scpool(result.getNAME(), result.getSYMBOL(), "888846", endDateStr, historytradeInfo.getTrade_date(), historytradeInfo.getOpen());
