...
@RestController
@RequestMapping("strategy88784")
public class Monitor4ASC815_22_88784 {
	@Autowired
	...
	@RequestMapping(value = "/monitor784",method = RequestMethod.POST)
	public List<StockCode> getFinal(String endDateStr) throws Exception
	{
			...
			if(stockbasics!=null)
			{
				...
				for(HistorytradeInfo historytradeInfo : resultList2Days ){
					...
					if(days4His==0){
						...
						if(historytradeInfo.getVol()<historytradeInfo.getV_ma5()*1.9 || historytradeInfo.getVol()>historytradeInfo.getV_ma5()*3.5){
							PERCENTASC = false;
							break;
						}
					}else{
		...
		for (StockCode result : Results) {
			System.out.println("Code:"+result.getSYMBOL()+"  name:"+result.getNAME()+ "   PE:"+result.getPE()  + "   MFRATIO2:"+result.getMFRATIO2()+ "    sore:"+result.getPROFIT()+ "    lowestPriceDays:"+result.getSYMBOL());
			Scpool scpool = new Scpool(result.getNAME(),result.getSYMBOL(), "88784",endDateStr,"",0.0);
